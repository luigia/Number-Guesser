...
submit = document.querySelector('#guess');
...
// make them global so they're not reset everytime the input is checked
let total = 1, remaining = 2;
...
function checkInput(e) {
  ...
  if (!inRange && userAnswer) {
    showError(`${userAnswer} is not in between ${min} and ${max}. Please enter a valid number.`);
    ...
    // user won
    showSuccess(`The correct answer was: ${answer} - You won! (${total } total tries and ${remaining} tries left)`);
    console.log(`The user used a total of ${total} tries with ${remaining} left to spare`);
    remaining--;
    total++;
...
function showError(message) {
  ...
  errorMsg ? errorMsg.textContent = message : form.appendChild(createErrorMsg);
}
...
function showSuccess(message) {
  ...
  successMsg ? successMsg.textContent = message : form.appendChild(createSuccessMsg);
}
...
function newGame() {
  ...
  const error = document.querySelector('#error-message'), success = document.querySelector('#success-message');
